  ...
  return function(scope, element, attrs) {
    ...
    attrs.$observe('ngSrc', function(value) {
      ...
      if (isRetina && typeof $window.sessionStorage === 'object') {
         // if ng-blind-2x attribute exists, blindly set the retina image without checking its existence
         // faster and supports cross server images
        if('ngBlind2x' in attrs){
           setImgSrc(getHighResolutionURL(value));
        }else{
           set2xVariant(value);
        }
      } else {
